...
- Security/audit: treat default high-risk node command IDs as recognized spellings for `gateway.nodes.denyCommands`, warn when those denies are redundant (not on default or configured allowlists), and skip redundant noise whenever `denyCommands` exactly matches the new-gateway wizard dangerous baseline (including with unrelated `allowCommands`). (#51865)
